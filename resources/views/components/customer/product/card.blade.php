...
@php
    ...
    // URL de la imagen
    $imageUrl = $product->hasImages() 
        ? $product->getFirstMediaUrl('default') 
        : null;
@endphp

<div {{ $attributes->merge(['class' => 'bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 ' . $size['container']]) }}>
    ...
    <div class="relative {{ $size['image'] }} bg-gray-100 overflow-hidden group">
        ...
        @else
            ...
            <div class="flex flex-col items-center justify-center h-full text-gray-400 bg-gradient-to-br from-gray-100 to-gray-200">
        ...
        @if($showQuickView)
            <div class="absolute bottom-3 left-1/2 transform -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <button 
                    type="button"
    ...
    <div class="{{ $size['padding'] }}">
        ...
        {{-- Nombre del Producto --}}
        <h3 class="font-roboto-flex {{ $size['title'] }} font-semibold text-gray-900 mb-2 line-clamp-2 min-h-[3.5rem] leading-tight hover:text-blue-600 transition-colors cursor-pointer">
            {{ $product->name }}
        </h3>
